...
DATA="$EXP_DIR/data"
SOURCE_TRAINING="$EXP_DIR/artifacts/training"
WARM_START="$EXP_DIR/artifacts/recovery_step3000_batch8/checkpoint.pt"
...
case "$RUN_MODE" in
  smoke)
    OUTPUT="$EXP_DIR/artifacts/recovery_smoke_step3300_lr1e6"
    TOTAL_STEPS=3304
    EVAL_EVERY=4
    PERIODIC_TRAIN_SAMPLES=4
    ;;
  formal)
    OUTPUT="$EXP_DIR/artifacts/recovery_step3300_batch8_lr1e6"
    TOTAL_STEPS=10000
    EVAL_EVERY=100
...
for path in \
  "$EXP_DIR" "$DATA" "$SOURCE_TRAINING" "$OUTPUT" "$LOGS" \
  "$CACHE" "$TMP"
do
...
"$ENVIRONMENT/bin/torchrun" --standalone --nproc-per-node=1 \
  ...
  --seed 101 >>"$LOGS/recovery_${RUN_MODE}.log" 2>&1
