...
DATA="$EXP_DIR/data"
WARM_START="$EXP_DIR/artifacts/training/checkpoint.pt"
...
case "$RUN_MODE" in
  smoke)
    OUTPUT="$EXP_DIR/artifacts/recovery_smoke_from_best3000_lr5e7"
    TOTAL_STEPS=3010
    EVAL_EVERY=10
    ...
    ;;
  formal)
    OUTPUT="$EXP_DIR/artifacts/recovery_from_best3000_lr5e7"
    TOTAL_STEPS=10000
    EVAL_EVERY=250
...
if [[ "$actual_sha" != "$EXPECTED_WARM_START_SHA" ]]; then
  echo "最佳检查点SHA-256不匹配：$actual_sha" >&2
  exit 95
fi
if [[ -f "$OUTPUT/report.json" ]]; then
  echo "$RUN_MODE 恢复分支已经完成，拒绝覆盖"
  exit 0
fi
if pgrep -af "moge.scripts.train_hypersim_joint_v3.*$OUTPUT" \
  | grep -v "$$" >/dev/null
then
  echo "$RUN_MODE 恢复进程已经存在，拒绝重复启动" >&2
  exit 6
fi
...
"$ENVIRONMENT/bin/torchrun" --standalone --nproc-per-node="$WORLD_SIZE" \
  ...
  --seed 111 \
  "${MAX_SAMPLE_ARGS[@]}" >>"$LOGS/recovery_${RUN_MODE}.log" 2>&1
