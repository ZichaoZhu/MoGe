...
EXP_DIR="$REPO/experiment/stage3_stability_normalization/runs/exp20_stateless_ssr_batch_statistics"
...
if [[ "$MODE" = formal ]]; then
  ...
  LIMIT_ARGS=(--max-samples-per-split 1)
fi
LOGS="$ROOT/logs"
install -d "$LOGS" "$(dirname "$STATUS")" "$CACHE/huggingface" \
...
status=0
if [[ ! -f "$ROOT/residual/report.json" ]]; then
  "$ENVIRONMENT/bin/python" tools/moge3/scan_checkpoint_residuals.py \
    ...
    "${LIMIT_ARGS[@]}" >>"$LOGS/residual.log" 2>&1 || status=$?
fi
