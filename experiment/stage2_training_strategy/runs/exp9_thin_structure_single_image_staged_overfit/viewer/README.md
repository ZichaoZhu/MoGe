...
- Exp30：Train 开放八张、Validation/Test 各开放五张此前人工确认的长细杆
  图片；Train 图片 6–8 与 Exp9 网站的样本 05、04、06 相同，可直接比较单图
  极限过拟合与百图共享训练；提供训练前、阶段一最佳和联合训练终点三个阶段；
...
Exp30 沿用 15 张人工确认的长细杆样本和锁定裁剪，并追加 Exp9 网站开放的三张
训练图，共保存 18 张图片的真实点云、训练前恒等点云、阶段一 K=0/1/3/5 与联合
终点 K=0/1/3/5。Train 在阶段一的 100 张图片上均有 K=3 改善；图片 6–8 分别是
Exp9 样本 05、04、06。Validation/Test 如实同时保留改善与退化案例。该入口主要
用于对照单图/百图训练以及“阶段一有效、联合阶段退化”，不作为留出域泛化成功
的证据。
