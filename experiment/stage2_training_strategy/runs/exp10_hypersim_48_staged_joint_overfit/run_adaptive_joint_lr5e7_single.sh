...
DATA="$EXP_DIR/data"
WARM_START="$EXP_DIR/artifacts/adaptive_joint_from_best3300_batch8_lr1e6/checkpoint.pt"
...
case "$RUN_MODE" in
  smoke)
    OUTPUT="$EXP_DIR/artifacts/adaptive_joint_lr5e7_smoke_from_best3500"
    TOTAL_STEPS=3510
    EVAL_EVERY=10
    PERIODIC_TRAIN_SAMPLES=4
    ;;
  formal)
    OUTPUT="$EXP_DIR/artifacts/adaptive_joint_lr5e7_from_best3500"
    TOTAL_STEPS=8500
    EVAL_EVERY=100
...
"$ENVIRONMENT/bin/torchrun" --standalone --nproc-per-node=1 \
  ...
  --seed 101 >>"$LOGS/adaptive_joint_lr5e7_${RUN_MODE}.log" 2>&1
