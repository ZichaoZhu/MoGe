...
BASELINE_HISTORY="$REPO/experiment/stage2_training_strategy/runs/exp15_detached_head_ssr_coadaptation/artifacts/formal/evaluation_history.csv"
...
case "$MODE" in
  ...
  formal)
    ...
    LIMIT_ARGS=()
    BASELINE_ARGS=(--baseline-history "$BASELINE_HISTORY")
    ;;
  *) exit 93 ;;
esac
...
for path in "$EXP_DIR" "$DATA" "$ROIS" "$BASELINE_HISTORY" "$ROOT" \
...
if [[ ! -f "$TRAINING/report.json" ]]; then
  "$ENVIRONMENT/bin/python" -m moge.scripts.train_hypersim_joint_v3 \
    ...
    --seed 151 \
    "${LIMIT_ARGS[@]}" >>"$LOGS/${MODE}_training.log" 2>&1 || status=$?
fi

if [[ "$status" -eq 0 && "$MODE" = formal && ! -f "$ROOT/evaluation/report.json" ]]; then
  "$ENVIRONMENT/bin/python" \
    ...
    --batch-size 1 \
    --boundary-threshold 0.03 \
    >>"$LOGS/${MODE}_evaluation.log" 2>&1 || status=$?
fi

if [[ "$status" -eq 0 && "$MODE" = formal && ! -f "$ROOT/residual/report.json" ]]; then
  "$ENVIRONMENT/bin/python" tools/moge3/scan_checkpoint_residuals.py \
    ...
    >>"$LOGS/${MODE}_residual.log" 2>&1 || status=$?
fi

if [[ "$status" -eq 0 ]]; then
  "$ENVIRONMENT/bin/python" "$EXP_DIR/summarize_results.py" \
    --root "$ROOT" --mode "$MODE" "${BASELINE_ARGS[@]}" \
    >>"$LOGS/${MODE}_summary.log" 2>&1 || status=$?
fi
