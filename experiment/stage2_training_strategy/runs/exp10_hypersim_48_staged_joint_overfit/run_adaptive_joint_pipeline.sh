...
RUNNER="$EXP_DIR/run_adaptive_joint_single.sh"
RESULT="$EXP_DIR/artifacts/logs/adaptive_joint_pipeline_result.json"
...
run_status=0
if [[ ! -f "$EXP_DIR/artifacts/adaptive_joint_smoke_from_best3300/report.json" ]]; then
  MOGE3_GPU="$GPU_INDEX" MOGE3_RUN_MODE=smoke bash "$RUNNER"
  run_status=$?
fi
if (( run_status == 0 )) \
  && [[ ! -f "$EXP_DIR/artifacts/adaptive_joint_from_best3300_batch8_lr1e6/report.json" ]]
then
