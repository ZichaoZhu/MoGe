...
test -f "$EXP_DIR/artifacts/training/checkpoint.pt"
install -d "$LOGS"

if pgrep -af "supervise_recovery.py.*$EXP_DIR" | grep -v "$$" >/dev/null; then
  echo "Exp11恢复监督器已经运行，拒绝重复启动" >&2
  exit 5
fi

cd "$REPO"
export PYTHONPATH="$REPO"
nohup "$ENVIRONMENT/bin/python" "$EXP_DIR/supervise_recovery.py" \
  --experiment-dir "$EXP_DIR" \
  --interval-seconds 600 \
  --min-free-mib 18000 \
  --max-utilization 30 \
  --max-gpus 2 >"$LOGS/recovery_supervisor_launcher.log" 2>&1 &
echo "$!" >"$LOGS/recovery_supervisor.pid"
echo "Exp11恢复监督器已启动，PID=$!"
