...
RUNNER="$EXP_DIR/run_adaptive_joint_lr5e7_single.sh"
RESULT="$EXP_DIR/artifacts/logs/adaptive_joint_lr5e7_pipeline_result.json"
...
run_status=0
if [[ ! -f "$EXP_DIR/artifacts/adaptive_joint_lr5e7_smoke_from_best3500/report.json" ]]; then
  MOGE3_GPU="$GPU_INDEX" MOGE3_RUN_MODE=smoke bash "$RUNNER"
  run_status=$?
fi
if (( run_status == 0 )) \
  && [[ ! -f "$EXP_DIR/artifacts/adaptive_joint_lr5e7_from_best3500/report.json" ]]
then
